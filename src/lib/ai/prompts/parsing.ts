...
export const PARSING_JSON_SCHEMA = `Return ONLY valid JSON (no markdown, no explanation):
{
  ...
  "skill_notes": [
    ...
    {"skill": "C++", "type": "scarce", "note": "C++ talent pools are thin in NZ — engineers with strong C or Rust backgrounds share the same systems-programming fundamentals and can typically ramp up quickly", "alternatives": ["C", "Rust"]}
  ],
...
- skill_notes: identify at most 3 skills that warrant a recruiter tip. Two distinct types:
  ...
  TYPE "scarce" — the skill is legitimate and modern but the NZ talent pool is genuinely constrained, AND there is a closely adjacent skill whose practitioners share the core mental model and can transfer with manageable ramp-up (NOT just any related language). The note should frame it as "pool is thin — consider adjacent engineers". Only generate this type when the skill is a PRIMARY must-have (not a nice-to-have), the NZ pool is objectively small, and the transferability is strong. Examples: C++ → C or Rust (systems programming fundamentals carry directly), embedded C → C++ or Rust, CUDA/GPU programming → C++ (HPC), Haskell/Erlang → Scala or F# (functional paradigm transfers), Fortran → C++/Python for scientific computing roles. Do NOT generate "scarce" for languages that have adequate NZ talent pools (Java, Python, JavaScript, C#, Go, SQL, PHP, Ruby, Swift, Kotlin, TypeScript, React, Angular, Vue, etc.).
