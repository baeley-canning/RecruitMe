...
export const PARSING_JSON_SCHEMA = `Return ONLY valid JSON (no markdown, no explanation):
{
  ...
  "search_queries": [
    "query 1: most common LinkedIn headline equivalent + the 1-2 most distinctive tech skills (e.g. 'Platform Engineer Kubernetes Terraform', 'Security Engineer penetration testing', 'Scrum Master agile coach')",
    "query 2: adjacent/synonym title that practitioners actually use on LinkedIn + key tech stack",
    "query 3: skills-first angle — the 2-3 most specific technical requirements without the job title (e.g. 'Kubernetes Terraform AWS', 'Snowflake dbt Python'). For hybrid/dual-function roles, target the OTHER dimension from queries 1-2.",
    "query 4: domain/industry angle, or a third title variant. For non-tech roles (PM, Scrum Master, Agile Coach): include practitioner-level synonyms people actually use ('Agile Coach', 'Delivery Lead', 'Release Train Engineer')."
  ],
  "google_queries": [
    "query 1: skills-first angle using the 2-3 most distinctive requirements. Always include at least one named technology or tool (not just a generic title). E.g. 'solutions architect Azure cloud', 'platform engineer Kubernetes', 'security engineer SIEM penetration testing', NOT just 'solutions architect'.",
    "query 2: skills-first angle for a different facet — if role is technical leadership, target the hands-on practitioners who are ready to step up; if hybrid, target the second domain."
  ],
...
- search_queries and google_queries: KEYWORD TERMS ONLY. Location and site:linkedin.com/in are added automatically. No years of experience. Never copy the exact job title verbatim. CRITICAL: every search_query must include at least one named technology, tool, or domain term from must_haves — a bare job title like "Platform Engineer" or "Solutions Architect" produces extremely broad results. Always add the key tech: "Platform Engineer Kubernetes Terraform", "Solutions Architect Azure microservices", "Security Engineer penetration testing SIEM". For non-tech roles (Scrum Master, Agile Coach, Product Manager): include domain-specific terms people actually put in LinkedIn headlines — "Scrum Master agile delivery", "Product Owner backlog prioritisation", "Agile Coach SAFe transformation".
