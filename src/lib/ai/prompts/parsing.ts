...
- anchor_terms: 2–5 specific technology or tool names that a strong candidate MUST have visible somewhere in their LinkedIn profile or headline. These drive candidate search filtering — they must be concrete, unambiguous terms (e.g. "C++", "Sybase", "Salesforce", "JMeter", "ServiceNow", "React", "Kubernetes"). Rules: only include terms from the must_haves list; never include soft skills, generic terms, methodologies, or degree requirements; if the role has no rare/distinctive technical anchor (e.g. a generic Project Manager role), return an empty array; maximum 5 terms. CRITICAL — terms to NEVER include as anchors because they are too broad and will pollute searches with unrelated candidates: "SQL" (use "SQL Server", "PostgreSQL", "Sybase", or "Oracle" instead), "Linux", "Python", "Java", "JavaScript", "cloud", "AWS", "Azure" alone, "API", "database", "agile", "git". Only include a language or tool if it is THE rarest/most-distinctive primary requirement — for C++/systems roles the anchor should be "C++"; for Salesforce roles "Salesforce"; do not pad the list with common supporting skills.
