...
- "unknown" = genuinely insufficient data — use for: (1) short snippets where absence is uninformative; (2) behavioural requirements (travel, driver's licence, criminal check) that profiles never explicitly state; (3) requirements that could exist anywhere in the candidate's background without being mentioned (e.g. soft skills). Do NOT default to "unknown" when the profile is long and the requirement is simply absent.
