...
- seniority_fit: score the DISTANCE from the target band, and treat OVER-qualification as a mismatch — not a bonus. This dimension is symmetric in distance but NOT in meaning: a candidate above the band will not accept the salary, will be bored, and will leave.
  ...
  * 0–20 = clearly beyond the role: currently Lead/Principal/Architect/Head-of/Manager/Director where the role is an individual contributor, or running teams where the role has no direct reports
  Judge the CURRENT level from recent titles and scope (team size, ownership, "leading a team of N"). Length of service does not by itself make someone a Lead — a long-tenured senior IC is still a senior IC.
  BUT tenure moves PRICE even when it does not move title, and price is what decides whether a hire is possible. When a budget ceiling is given, infer the candidate's likely current compensation from total relevant experience, current level and scope, then score against that ceiling:
  * A senior IC with roughly 12+ years of relevant experience sits near the top of the senior market and commands well above a mid-band senior salary. Against a ceiling at or below that band, that is over-qualification — score 0–30 — EVEN THOUGH the title matches exactly.
  * The test is never "could they do this job". It is "would they accept this number, and still be here in a year". Someone who would take a pay cut to join will usually decline, and will leave if they don't.
  * Say the inference out loud in the evidence, with the numbers: e.g. "15 years at senior level — likely $150k+, above the $125k ceiling."
  * If NO budget ceiling is given, ignore compensation entirely and score level distance only. Never invent a ceiling that the role did not state.
