...
export default async function PublicShortlistPage({
  ...
  const { job, candidates, brand } = data;
  const brandName = brand?.brandName || "RecruitMe";

  return (
    <div className="min-h-screen bg-surface-base">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        {/* Brand lockup (white-label) */}
        <div className="flex items-center gap-2 mb-5">
          {brand?.logoUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={brand.logoUrl} alt="" className="h-6 w-auto max-w-[140px] object-contain" />
          ) : null}
          <span className="text-sm font-semibold text-text-secondary">{brandName}</span>
        </div>
        {/* Header */}
        <div className="mb-6">
        ...
        <p className="mt-10 text-center text-xs text-text-tertiary">
          {brand?.footerText || `Read-only shortlist · Generated by ${brandName}`}
        </p>
      </div>
    </div>
  );
